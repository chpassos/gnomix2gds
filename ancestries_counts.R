...
lai_inferences <-
  read_delim("/home/chpassos/Analises/REDS-III/ancestries/lai/chr2_output/query_results.msp",
             skip = 1)
...
# Read this file
pos <- 
  read.table("/home/chpassos/Analises/REDS-III/ancestries/lai/chr2_output/chr2_info.txt") |>
  mutate(chr = "2") |>
  rename("rsid" = "V1",
         "position" = "V2",
         "ref" = "V3",
...
afr_part1_unglued <- afr_part1_joined |>
  select(-c(`#chm`)) |>
  select(rsid, position, chr, ref, alt, everything())
afr_part2_unglued <- afr_part2_joined |>
  select(-c(`#chm`, chr, position, spos, epos, sgpos, 
            egpos, `n snps`, rsid, position, chr, ref, alt))
...
afr_glued |>
  write_csv("/home/chpassos/Analises/REDS-III/ancestries/lai/admixture_mapping/gds/chr8_afr_count.tsv")

eur_glued |>
  write_csv("/home/chpassos/Analises/REDS-III/ancestries/lai/admixture_mapping/gds/chr8_eur_count.tsv")

nam_glued |>
  write_csv("/home/chpassos/Analises/REDS-III/ancestries/lai/admixture_mapping/gds/chr8_nam_count.tsv")




## Get rsID in VCF file used in Gnomix
# bcftools query -f '%ID\t%POS\t%REF\t%ALT\n' REDS_chr2_gnomix.vcf.gz > chr2_info.txt



afr_gen <- afr_glued |> 
  select(-c(rsid, position, chr, ref, alt, 
            spos, epos, egpos, sgpos, `n snps`)) |>
  as.matrix()
afr_id <- id_order
afr_rsid <- afr_glued |>
  select(rsid) |>
...
afr_pos <- afr_glued |>
  select(position) |>
  pull()
afr_allele <- afr_glued |>
  select(ref, alt) |>
  unite(allele, ref, alt, sep = "/") |>
  pull()
...
snpgdsCreateGeno("test.gds", 
                 ...
                 sample.id = id_order, 
                 snp.id = afr_rsid,
                 snp.chromosome = afr_chr,
                 snp.position = afr_pos,
                 snp.allele = afr_allele,
                 snpfirstdim=TRUE)
genofile <- snpgdsOpen("test.gds")

getGenotype(genofile)
snpgdsClose(genofile)

chr2_reds_gds <- GdsGenotypeReader("test.gds")
getScanID(chr2_reds_gds)
GWASTools::getGenotype(chr2_reds_gds)[,8]
